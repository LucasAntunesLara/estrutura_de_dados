...
const disableBtn = () => (actionToggleBtn.disabled = numberInput.value === '')
...
actionToggleBtn.addEventListener('keydown', e => {
  if (['Enter', ' '].includes(e.key)) {
    e.preventDefault()
    toggleMenu(actionMenu, actionToggleBtn)
  } else if (e.key === 'Escape') closeMenu(actionMenu, actionToggleBtn)
})
...
actionDeleteToggle.addEventListener('keydown', e => {
  if (['Enter', ' '].includes(e.key)) {
    e.preventDefault()
    toggleMenu(actionDeleteMenu, actionDeleteToggle)
  } else if (e.key === 'Escape') closeMenu(actionDeleteMenu, actionDeleteToggle)
})
